...
- `updateFile()` writes to a sibling swap file named `{path}.temp-{pid}-{token}` (opened `wx`, `token` is 8 base64url characters, 48 bits, from `randomBytes(6)`), preserves the target's mode across the swap, then renames it over the target. The name was a whole-second timestamp until #44, which made concurrent saves on one path collide. Concurrent write scenarios are covered in `test/unit/file-concurrency-test.ts` -- that suite forces the interleaving with `sinon.useFakeTimers({ toFake: ['Date'] })` plus a barrier stubbed onto `fs.promises.writeFile` with `sinon.stub`; a bare `Promise.all` is a sample, not a test
- Stub `fs.promises` through sinon, never by assignment. It is a process-global object and QUnit runs one process, so an unrestored patch surfaces as a failure in an unrelated test file. The `sinon.restore()` in `afterEach` only helps if sinon owns the patch
