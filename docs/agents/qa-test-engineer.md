...
- `updateFile()` writes to a sibling swap file named `{path}.temp-{pid}-{token}` (opened `wx`, `token` is 8 hex chars from `randomUUID()`), preserves the target's mode across the swap, then renames it over the target. The name was a whole-second timestamp until #44, which made concurrent saves on one path collide. Concurrent write scenarios are covered in `test/unit/file-concurrency-test.ts` -- that suite forces the interleaving with `sinon.useFakeTimers({ toFake: ['Date'] })` plus a barrier stubbed onto `fs.promises.writeFile` with `sinon.stub`; a bare `Promise.all` is a sample, not a test
