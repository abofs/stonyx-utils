...
- Atomic file updates: `updateFile()` writes to a sibling swap file suffixed `{pid}-{token}` (opened `wx`), then renames. The swap file must stay a sibling of the target so `rename` remains same-filesystem and therefore atomic. Contract is last-writer-wins: concurrent callers never corrupt each other, but they are not serialized. The swap file is cleaned up on every failure path the function controls, except an `EEXIST` raised by the `wx` *open* -- that path belongs to another writer. Ownership is decided by stage and code together, never by the code alone: an `EEXIST` from `rename` follows a successful open, so that swap file is ours and is unlinked. A process killed between write and rename leaves a `*.temp-*` orphan that nothing reclaims
